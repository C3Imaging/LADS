...
class LADS:
    def __init__(self, H, W, device, ts_to_seconds_factor=1, num_bins=1, 
                 ...
                 recursive=False, min_patch_size=0, activity_threshold=0.5, stop_above_thresh=True):
        ...
        if self.do_patch_decay and self.patch_size is not None:
            ...
            self.patch_activity_conv = torch.nn.Conv2d(1, 1, kernel_size=self.patch_size, stride=self.patch_size, padding=0, bias=False, device=device)           
            self.patch_activity_conv.weight.data.fill_(1)
            self.patch_activity_conv.weight.requires_grad = False
        ...
        if self.decay_func == "log":
            ...
            self.patch_activity_conv.weight.data.fill_(1/(self.patch_size[0]*self.patch_size[1]))
    ...
    def calc_patch_event_rates(self, events, time_diff_s):
        activity_voxel_grid = voxel(events, self.H, self.W, self.device, polarity_mapping=(1, 1))
        patch_event_counts = self.patch_activity_conv(activity_voxel_grid).squeeze(0)
    ...
    def decay_by_event_rate_exp(self, events, time_diff_s, use_patches=False):
        ...
        if not use_patches:
            ...
            activity = event_rate / self.reference_event_rate
            decay_factor = np.exp((-(self.decay*activity)))
            return decay_factor, activity
        else:
            if time_diff_s <= 0:
            ...
            patch_activities = patch_event_rates/self.reference_event_rate
            # decay scales exonentially with activity, but unlike event-rate-linear, does not hit 1 when equal to reference rate
            # when equal to reference rate, decay is exp(-decay)
            patch_decay_factors = torch.exp(((-time_diff_s*patch_activities)/self.decay))
            return patch_decay_factors, patch_activities
    ...
    def decay_by_event_rate_linear(self, events, time_diff_s, use_patches=False):
        if not use_patches:
            ...
            event_rate = len(events) / time_diff_s
            activity = event_rate / self.reference_event_rate
            decay_factors = 1 - activity
            return decay_factors, activity
        else:
            if time_diff_s <= 0:
                return torch.ones((self.H//self.patch_size[0], self.W//self.patch_size[1]), device=self.device), \
                       torch.zeros((self.H//self.patch_size[0], self.W//self.patch_size[1]), device=self.device)
            patch_event_rates = self.calc_patch_event_rates(events, time_diff_s)
            patch_activities = patch_event_rates/self.reference_event_rate
            patch_decay_factors = 1 - patch_activities
            return patch_decay_factors, patch_activities
    ...
    def decay_by_LoG(self, new_grid):
        activities = torch.abs(conv2d(new_grid.unsqueeze(0).unsqueeze(0), self.laplace_kernel_025, padding=1))
        if self.patch_size[0] == 1 and self.patch_size[1] == 1:
            patch_activities = activities.squeeze(0).squeeze(0)
        else:
            patch_means = self.patch_activity_conv(activities).squeeze(0).squeeze(0)
            patch_activities = patch_means
        
        a = self.falloff_rate
        b = self.decay#*100
        patch_decay_factors = (1.0 + np.exp(-a*(b))) / (1.0 + torch.exp(a * (patch_activities - b)))
        return patch_decay_factors, patch_activities
    ...
    def decay_by_fft(self, activity_voxel_grid):

        if self.patch_size is None:
            if self.recursive:
                patch_activities = subdivide_grid_recur(activity_voxel_grid, min_patch_size=self.min_patch_size, activity_function=self.get_fft_fraction,
                                                        activity_threshold=self.activity_threshold, stop_above_thresh=self.stop_above_thresh)
            else:
            ...
            patches = activity_voxel_grid.unsqueeze(0).unsqueeze(0).unfold(2, self.patch_size[0], self.patch_size[0]).unfold(3, self.patch_size[1], self.patch_size[1]).contiguous()
            patches = patches.view(1,1,ny,nx,self.patch_size[0],self.patch_size[1])
                
            if self.recursive:
                patch_activities = self.recurr_with_init_patches(patches)

            else:
                patch_activities = torch.zeros((ny, nx), device=self.device)
                for i in range(ny):
                    for j in range(nx):
                        new_grid_patch = patches[:,:,i,j]
                        patch_activity = self.get_fft_fraction(new_grid_patch)
                        patch_activities[i,j] = patch_activity

        return patch_activities
        ...
        with torch.no_grad():
            ...
            if not self.do_patch_decay:
                if self.decay_func == "er":
                    decay_factor, activity = self.decay_by_event_rate_exp(events, time_diff_s, use_patches=False)
                    decay_factor = torch.tensor(decay_factor, device=self.device).clamp(0, 1-self.min_decay)
                    return self._update_grid(decay_factor, new_events=events)
                
                elif self.decay_func == "event-rate-linear":
                    decay_factor, activity = self.decay_by_event_rate_linear(events, time_diff_s, use_patches=False)
                    decay_factor = torch.tensor(decay_factor, device=self.device).clamp(0, 1-self.min_decay)
                    return self._update_grid(decay_factor, new_events=events)
            ...
            if self.decay_func == "er":
                patch_decay_factors, patch_activities = self.decay_by_event_rate_exp(events, time_diff_s, use_patches=True)


            elif self.decay_func == "er-linear":
                patch_decay_factors, patch_activities = self.decay_by_event_rate_linear(events, time_diff_s, use_patches=True)


            elif self.decay_func == "log":
                patch_decay_factors, patch_activities = self.decay_by_LoG(new_grid)


            elif self.decay_func == "fft":
                patch_activities = self.decay_by_fft(new_grid)
                patch_decay_factors = patch_activities
            ...
            if self.interpolate_patches:
                ...
                patch_decay_factors = patch_decay_factors.repeat_interleave(self.H//patch_activities.shape[-2], dim=0)
                patch_decay_factors = patch_decay_factors.repeat_interleave(self.W//patch_activities.shape[-1], dim=1)
            ...
            if return_patch_features:
                return self.old_grid, (patch_activities, patch_decay_factors)
    ...
    def recurr_with_init_patches(self, patches):
        ...
        for i in range(ny):
            for j in range(nx):
                ...
                patch_activity = self.get_fft_fraction(new_grid_patch)
                if (patch_activity < self.activity_threshold and self.stop_above_thresh) or \
                    (patch_activity > self.activity_threshold and not self.stop_above_thresh):
                    if not (ph%2 != 0 or pw%2 != 0 or ph/2 < self.min_patch_size or pw/2 < self.min_patch_size): 

                        patch_activity = subdivide_grid_recur(new_grid_patch[0,0], min_patch_size=self.min_patch_size, 
                                                                activity_function=self.get_fft_fraction,
                                                                activity_threshold=self.activity_threshold, 
                                                                stop_above_thresh=self.stop_above_thresh)
                new_powers.append(patch_activity)
                new_is.append((i,j))
        ...
        patch_activities = torch.zeros((max_shape*ny,max_shape*nx), device=self.device)
        for i in range(ny):
            for j in range(nx):
                patch_activities[max_shape*i:max_shape*(i+1),max_shape*j:max_shape*(j+1)] = new_powers[new_is.index((i,j))]
        return patch_activities
